...
- [P1] Finish the last English-study closeout slice: continue from the new `web/dist` vs `web/src`, `core/static` shell-vs-sample, and `annotation_templates` vs `core/examples` split, then decide which remaining upstream-restored `label_studio/` assets still need long-term retention.
