...
- Current split finding: `assistant-state-backup` did contain tracked `workspace/projects/english-study/repo/...` files, the standalone `projects/english-study/repo/.git` was broken, and it has now been replaced with a healthy remote-derived `.git` so the local English-study tree is again a real git repository. The local long-term branch shape is tighter now too: `assistant-state-backup` local `main` now tracks `origin/workspace-main`, and `projects/english-study/repo` has been switched back to local `main` tracking `origin/main`; the remaining cleanup is whether to retain/hide the recovery branches and how to document the default future push flow.
