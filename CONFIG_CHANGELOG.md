...
### `context-safe` plugin staged into canonical runtime state

- Found a local offline source tree at `/mnt/d/openclaw-context-safe-plugin-20260313`
- Reviewed the manifest and README:
  - plugin id: `context-safe`
  - kind: `context-engine`
  - compatibility claim: `OpenClaw >= 2026.3.8`
  - implementation uses official plugin extension points (`before_tool_call`, `tool_result_persist`, `plugins.slots.contextEngine`)
- Copied the plugin tree into the canonical runtime extension root:
  - `var/config/extensions/context-safe`
- Updated live persistent config:
  - `plugins.entries.context-safe.enabled = true`
  - `plugins.slots.contextEngine = "context-safe"`

Important note:

- No Docker compose change was needed because the existing runtime already bind-mounts `var/config` as the OpenClaw state dir, and the default plugin discovery root includes `$OPENCLAW_STATE_DIR/extensions`
- Runtime restart / live verification could not be executed from this sandbox because Docker daemon access is denied here

### `openclaw-customprovider-cache` remains blocked

- No local offline source tree for `openclaw-customprovider-cache` was found
- Direct GitHub fetch is currently blocked by the host proxy / restricted network path in this sandbox
- Therefore that second plugin was **not** installed in this pass

## 2026-03-20

### Both requested third-party plugins are now staged into canonical runtime state

- User later provided refreshed local source trees:
  - `/mnt/d/openclaw-context-safe-plugin-20260313` at commit `81a61ce`
  - `/mnt/d/openclaw-customprovider-cache` at commit `2c7532c`
- Staged both plugins into the canonical state extensions root:
  - `var/config/extensions/context-safe`
  - `var/config/extensions/openclaw-customprovider-cache`
- Updated persistent runtime config in `var/config/openclaw.json`:
  - `plugins.entries.context-safe.enabled = true`
  - `plugins.slots.contextEngine = "context-safe"`
  - `plugins.entries.openclaw-customprovider-cache.enabled = true`
  - `plugins.entries.openclaw-customprovider-cache.config.providers = ["rightcode", "foxcode-ultra", "foxcode-aws"]`

Validation:

- JSON parse check on `var/config/openclaw.json` passed
- Config lookup confirms both plugins are enabled and `context-safe` is the active context engine slot

Limit:

- current Codex session still cannot access Docker daemon, so runtime recreate and `openclaw plugins info/doctor` live verification remain pending

### Gateway token change now requires container recreate, not just restart

- User-side change: `OPENCLAW_GATEWAY_TOKEN` has been commented out in `.env`
- In this Docker topology, `.env` changes only take effect after container recreate
- Required next step remains:
  - recreate the OpenClaw runtime containers via the canonical runtime script
  - then re-run operator-auth regression checks

### Current Codex session cannot execute Docker live actions

- This session can still edit files under the repo and stage runtime config
- But current sandbox denies access to the Docker daemon socket, so live `docker compose exec/up` verification is blocked here
- Result:
  - file-level runtime prep can be completed
  - rebuild / recreate / live verification must run from a session with Docker daemon access
...
## 2026-03-21 01:55 (北京时间)
- `channels.feishu.streaming` → `false`：关闭飞书 streaming card，修复消息投递静默失败问题
- 原因：streaming card 启动失败时无 fallback 到普通消息，导致回复丢失
